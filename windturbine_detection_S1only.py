...
class WindturbineDetector():
    ...
    def __init__(self, selection_windturbine_paths=[""], selection_no_windturbine_paths=[""], 
                 ...
                 input_shape=[30, 30, 2], pool_size=2, strides=2, full_connection_units=128, 
                    ...
                    for crop in tqdm(crop_directories, desc=f"Scanning crops [{category.name}]:"):
                        if crop.is_dir() and crop.name != "0_combined-preview":
                            ...
                            for band in self.image_bands:
                                ...
                                if band == "VV":
                                    image_file = list(image_path.glob("*VV.tif"))[0]
                                elif band == "VH":
                                    image_file = list(image_path.glob("*VH.tif"))[0]

                                if image_file.exists():
                                    with rasterio.open(str(image_file.absolute())) as f:
                                        if image_list.size == 0:
                                            image_list = f.read(indexes=1) + self.value_increment
                                        else:
                                            image_list = np.dstack((image_list, ( f.read(indexes=1) + self.value_increment )))                                        
